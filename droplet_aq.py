## calibration factor for aqueous droplet experiment
import numpy as np
import os
import time
import datetime
...
def aqueous_droplet(fnr, gas, cid, weight, MW, t1, t2, t3, pct=4, row=500, showgraph=False, savefig=False):
    ...
    x1 = h('time')
    y1 = h(gas_name) * 1e6
    z1 = h('broadband_gasConcs_962') * 1e6   #water
    ...
    baseline3 = z[idx2]           # water
    zero3 = np.mean(baseline3)
    std3 = np.std(baseline3)
    print('water zero3, std3:', zero3, std3)
    ...
    # print(i)
    # print(len(x))
    z0 = z[tbb:i]-zero3        ##truncated z, zero-ed
    ...
    for j in range(len(y0)-1):
        ...
        #water
        dZ = 0.5 * (z0[j + 1] + z0[j])
        moles = dZ * mflow_tot * dt
        w += moles    # micromoles
        W.append(w)

    weight2 = float(W[-1]) * 18.01528 * 0.000001   #water weight
    weight1 = float(weight) - weight2      #analyte weight
    print('water weight: ')
    print(weight2)
    print('analyte weight: ')
    print(weight1)

    vol_in = float(weight1) / float(MW)
    ...
    A.plot(x, y, label=' cid = %s (ppm)' % cid)
    A.plot(x, z, label=' water (ppm)')
    ...
    data, _, max_row = read.get_spectra_row('broadband', row, pull_results=True)[0]
    nu = data['nu']
    k = data['absorbance']
...
if __name__ == "__main__":
    ...
    if not os.path.exists(fnr):
        ...
        # aqueous_droplet(fnr, gas, cid, weight, MW, t1, t2, t3, row=row1, showgraph=True)
        aqueous_droplet(fnr, gas, cid, weight, MW, t1, t2, t3, row=row1, savefig=True)
