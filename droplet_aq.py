...
def aqueous_droplet(fnr, gas, cid, weight, MW, t1, t2, t3, row=500, pct=4, showgraph=False, savefig=False):
    ...
    # water
    baseline = z1[idx]
    zero3 = np.mean(baseline)
    std3 = np.std(baseline)
    ...
    print('water weight: ', weight2)
    print('analyte weight: ', weight1)
    ...
    A3 = A1.twinx()
    A3.plot(x, MFC2, label='Beaker', color='#000088')   #dkblue
...
if __name__ == "__main__":
    ...
    if not os.path.exists(fnr):
        ...
        # aqueous_droplet(fnr, gas, cid, weight, MW, t1, t2, t3, row1, showgraph=True, savefig=True)
